...
class BRATS_DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, patient_IDs, train=True, batch_size=1, dim=(192,192,192), shuffle=False, n_channels=4, augment_flip=False, seed=2020, n_labels=3, segment=True, structure=None):
        'Initialization'
        self.dim = dim
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            t1 = nib.load('/mnt/dsets/brats/' + subdir + ID + '/' + ID + '_t1.nii.gz').get_data()
            t1 =  padcrop(t1, self.dim) 
            t1ce = nib.load('/mnt/dsets/brats/' + subdir + ID + '/' + ID + '_t1ce.nii.gz').get_data()
            t1ce = padcrop(t1ce, self.dim)
            t2 = nib.load('/mnt/dsets/brats/' + subdir + ID + '/' + ID + '_t2.nii.gz').get_data()
            t2 = padcrop(t2, self.dim)
            flair = nib.load('/mnt/dsets/brats/' + subdir + ID + '/' + ID + '_flair.nii.gz').get_data()
            ...
            if self.segment:
                seg = nib.load('/mnt/dsets/brats/' + subdir + ID + '/' + ID + '_seg.nii.gz').get_data().astype(np.int)
                seg[seg == 4] = 3 
                ...
                seg2 = np.isin(seg, 2.)
                seg3 = np.isin(seg, 3.)
                segs = np.stack((seg0, seg1, seg2, seg3))
